...
const regex = /const formData = new FormData\(\);[\s\S]*?\}\)\(\)\s*\}\)\.catch\(\(\) => \{\}\)\);/; // wait, I will just use string index replacement

const startIndex = txt.indexOf('const formData = new FormData();\n         formData.append(&quot;file&quot;, file);\n         fetch(\'/api/drive/upload\'');
const endIndexStr = 'status.innerHTML = oldHtml + &quot;<br><span style=\'color:#b45309;\'>❌ <b>Lỗi khi đẩy file lên Google Drive:</b> &quot; + (err.message || err.toString() || \'Lỗi không xác định\') + &quot;</span>&quot;;\n         });';
const endIndex = txt.indexOf(endIndexStr, startIndex) + endIndexStr.length;
...
if (startIndex !== -1 && endIndex !== -1) {
    ...
    txt = txt.substring(0, startIndex) + newFetchBlock + txt.substring(endIndex);
    fs.writeFileSync('views/app.ejs', txt);
    console.log("Patched chunked upload via index replacement!");
} else {
